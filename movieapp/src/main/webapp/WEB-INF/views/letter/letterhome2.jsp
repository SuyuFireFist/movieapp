...
<div class="container">          
  <table class="table table-hover" style="text-align:center;">
    ...
    <c:if test = "${not empty page.getList() }">
      ...
      <tr>
      	...
      	<c:if test="${a.readyn=='N'}">
      	<td></td>
      	</c:if>
      	<c:if test="${a.readyn=='Y'}">
      	<td>${a.read_date}</td>
      	</c:if>
      </tr>
      </c:forEach>
  ...
  <ul class="pagination">
    ...
    <li class="page-item">
      <a class="page-link" href="<%=request.getContextPath()%>/lettermysend?user_id=${sessionScope.SPRING_SECURITY_CONTEXT.authentication.principal.username}&&currentPage=${page.startPage-1}" aria-label="Previous">
        <span aria-hidden="true">&laquo;</span>
      </a>
    </li>
    </c:if>
    <c:forEach begin="${page.getStartPage()}" end="${page.getEndPage()}" var="i" step="1">
    <li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/lettermysend?user_id=${sessionScope.SPRING_SECURITY_CONTEXT.authentication.principal.username}&&currentPage=${i}">${i}</a></li>
    </c:forEach>
    <c:if test="${page.totalPage != page.endPage }">
    <li class="page-item">
      <a class="page-link" href="<%=request.getContextPath()%>/lettermysend?user_id=${sessionScope.SPRING_SECURITY_CONTEXT.authentication.principal.username}}&&currentPage=${page.endPage+1}" aria-label="Next">
